...
/* ************************************************** */
/* ************************************************** */
int callmeback(void* ws, call_t *c, void *args);
int tx(void* ws, call_t *c, void *args);

void init_adj(wsnet_t* ws, call_t *c);
void show_adj(wsnet_t* ws, call_t *c);
void save_adj(wsnet_t* ws, call_t *c);

int get_neighbors_nbr(wsnet_t* ws, call_t *c, int node);
double get_max_neighbor(wsnet_t* ws, call_t *c, int node);
double get_min_neighbor(wsnet_t* ws, call_t *c, int node);
int is_connex(wsnet_t* ws, call_t *c);
int get_nbrconnex(wsnet_t* ws, call_t *c);
...
int init(wsnet_t* ws, call_t *c, void *params) {
    ...
    entitydata->nbr_nodes = ws->get_node_count();
...
int destroy(wsnet_t* ws, call_t *c) {
    struct _hello_entity_private *entitydata = ws->get_entity_private_data(c);
    int i = 0;
    for (i = 0; i < entitydata->nbr_nodes; i++) {
...
int setnode(wsnet_t* ws, call_t *c, void *params) {
    ...
    int i = ws->get_entity_links_down_nbr(c);
    param_t *param;
    ...
    while ((param = (param_t *) ws->das_traverse(params)) != NULL) {
        ...
        if (!strcmp(param->key, "period")) {
            if (ws->get_param_time(param->value, &(nodedata->w))) {
                goto error;
            }
        }
        ...
        if (!strcmp(param->key, "graph")) {
            if (ws->get_param_integer(param->value, &(nodedata->graph))) {
                goto error;
            }
        }
    }
...
int unsetnode(wsnet_t* ws, call_t *c) {
    struct _hello_private *nodedata = ws->get_node_private_data(c);
...
int bootstrap(wsnet_t* ws, call_t *c) {
    ...
    uint64_t start = ws->get_time();
    int n =0, source = 0;
    double dist = 999999, tmp = 0;
    position_t center = {ws->get_topology_area()->x/2, ws->get_topology_area()->y/2, ws->get_topology_area()->z/2};

    /* get overhead */
    while (i--) {
        call_t c0 = {down[i], c->node, c->entity};
        if ((ws->get_entity_type(&c0) != MODELTYPE_ROUTING)
            && (ws->get_entity_type(&c0) != MODELTYPE_MAC)) {
            nodedata->overhead[i] = 0;
        } else {
            nodedata->overhead[i] = ws->GET_HEADER_SIZE(&c0);
        }
    }
    ...
    if (c->node == 0) {
      n = ws->get_node_count();
      for (i=0; i<n; i++) {
	tmp = ws->distance(ws->get_node_position((nodeid_t)i), &center);
	if (tmp < dist) {
	  dist = tmp;
	  source = i;
	}
...
int callmeback(void* wsv, call_t *c, void *args) {
    ...
    uint64_t start = ws->get_time() + ws->get_random_double() * (nodedata->w - nodedata->d);
    ws->scheduler_add_callback(start, c, tx, NULL);
 
    
    if (c->node == entitydata->source) {
      int neighbors_nbr = get_neighbors_nbr(ws,c, c->node);
      double max_neigh  = get_max_neighbor(ws,c, c->node);
      int connexity = get_nbrconnex(ws,c);
      
      time = ws->get_time() * 0.000000001;
    ...
    ws->scheduler_add_callback(ws->get_time() + nodedata->w, c, callmeback, NULL);
    return 0;
}

/* ************************************************** */
/* ************************************************** */
int tx(void* wsv, call_t *c, void *args) {
	wsnet_t* ws = (wsnet_t*) wsv;
    struct _hello_private *nodedata = ws->get_node_private_data(c);
    struct _hello_entity_private *entitydata =  ws->get_entity_private_data(c);
    ...
    while (i--) {
      packet_t *packet = ws->packet_create(c, nodedata->size + sizeof(struct _hello_header) + nodedata->overhead[i], -1);
        call_t c0 = {down[i], c->node, c->entity};
...
void save_adj(wsnet_t* ws, call_t *c) {
    ...
    FILE *file = NULL;
    char file_map[100];
    double time = ws->get_time() * 0.000001;
    int i = 0, j = 0, n = 0;
    double degree = 0;
    ...
    for (i = 0; i < entitydata->nbr_nodes; i++) {
        fprintf(file, "%d %lf %lf", i, ws->get_node_position(i)->x, ws->get_node_position(i)->y);
        for (j = 0; j < entitydata->nbr_nodes; j++) {
...
double get_max_neighbor(wsnet_t* ws, call_t *c, int node) {
    ...
    for (i = 0; i < entitydata->nbr_nodes; i++) {
      if (i != node && entitydata->adj[node][i] == 1) {
	tmp = ws->distance(ws->get_node_position(node), ws->get_node_position(i));
	if (tmp > dist) {
	  dist = tmp;
	}
      }
...
int is_connex(wsnet_t* ws, call_t *c) {
  ...
  void  *pile =(uintptr_t *) ws->das_create();
  ...
  v[0] = 1;
  ws->das_insert(pile, 0);

  while (ws->das_getsize(pile) != 0) {
    n = (uintptr_t) ws->das_pop(pile);
    v[n] = 1;
    for (j=0; j<n; j++) {
      if (entitydata->adj[n][j] == 1 && v[j] == 0) {
    	  ws->das_insert(pile,(void *)(uintptr_t) j);
	v[j] = 1;
      }
    }
  }
...
int get_nbredge(wsnet_t* ws,call_t *c, int node, int *v) {
  struct _hello_entity_private *entitydata =  ws->get_entity_private_data(c);
  void *stack =(uintptr_t  * ) ws->das_create();
  int n = 0, tmp = 0, i = 0;
  
  ws->das_insert(stack,(void *)(uintptr_t) node);

  while(ws->das_getsize(stack)!=0) {
    tmp = (uintptr_t) ws->das_pop(stack);
    v[tmp] = 1;
	...
	if (v[i] == 0) {
	  v[i] = 1;
	  ws->das_insert(stack,(void *)(uintptr_t)i);
	}
      }
    }
  }
