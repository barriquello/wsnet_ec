...
int medium_bootstrap(void) {
    ...
    propagation_entity->library.module = g_module_open(propagation_entity->library.file, G_MODULE_BIND_LOCAL);
    ...
    if (propagation_entity->bootstrap) {
        propagation_entity->bootstrap(&wsnet, &c);
    }   
    g_module_close(propagation_entity->library.module);
    ...
    if (shadowing_entity) {
        ...
        if (shadowing_entity->bootstrap) {
            shadowing_entity->bootstrap(&wsnet, &c);
        }   
    }
    ...
    if (fading_entity) {
        ...
        if (fading_entity->bootstrap) {
            fading_entity->bootstrap(&wsnet, &c);
        }   
    }
...
void medium_tx_end(packet_t *packet, call_t *c) {
    ...
    if (node->state == NODE_ACTIVE) {
        ENTITY_OPEN(entity);
        entity->methods->radio.tx_end(&wsnet, c, packet);
        ENTITY_CLOSE(entity);
    }
...
void medium_cs(packet_t *packet, call_t *c) {
    ...
    /* start reception */
    ENTITY_OPEN(entity);
    entity->methods->antenna.cs(&wsnet, c, packet);
    ENTITY_CLOSE(entity);
    /* end reception */
...
void medium_compute_rxdBm(packet_t *packet, call_t *c) {
    ...
    if (c->node != packet->node) {
        ENTITY_OPEN(propagation_entity);
        rxdBm = propagation_entity->methods->propagation.propagation(&wsnet, &c0, packet,
                                                                     packet->node,
                                                                     c->node, rxdBm);
        ENTITY_CLOSE(propagation_entity);
    }
