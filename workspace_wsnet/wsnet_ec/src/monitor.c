...
int monitor_bootstrap(void) {
    ...
    if (monitor_entity) {
        monitor_entity->library.module = g_module_open(monitor_entity->library.file, G_MODULE_BIND_LOCAL);
        ...
        if (monitor_entity->bootstrap) {
            monitor_entity->bootstrap(&wsnet, &c);
        }   


        g_module_close(monitor_entity->library.module);
    }
