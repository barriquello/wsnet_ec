...
int noise_bootstrap(void) {
    ...
    interference_entity->library.module = g_module_open(interference_entity->library.file, G_MODULE_BIND_LOCAL);
    ...
    if (interference_entity->bootstrap) {
        interference_entity->bootstrap(&wsnet, &c);
    }   
    
    g_module_close(interference_entity->library.module);
    ...
    if (noise_entity) {
        ...
        if (noise_entity->bootstrap) {
            noise_entity->bootstrap(&wsnet, &c);
        }   
    }
    ...
    for (i = 0; i < CHANNELS_NUMBER; i++) {
        for (j = 0; j < CHANNELS_NUMBER; j++) {
            correlation[i][j] = interference_entity->methods->interferences.interfere(&wsnet, &c, i, j);
        }
    }
    g_module_close(interference_entity->library.module);
