...
void do_end(void) {
    ...
    while (i--) {
        ...
        for (j = 0; j < bundles.elts[node->bundle].entity.size; j++) {
            ...
            entity->library.module = g_module_open(entity->library.file, G_MODULE_BIND_LOCAL);
            call_t c = {entity->id, node->id, -1};
            if ((entity)->unsetnode) {
                (entity)->unsetnode(&wsnet, &c);
            }
...
void do_event(event_t *event) {
    ...
    case PRIORITY_CALLBACK:
        ...
        entity_t* entity = get_entity_by_id(event->u.cb.call.entity);
        entity->library.module = g_module_open(entity->library.file, G_MODULE_BIND_LOCAL);
        event->u.cb.callback(&wsnet, &(event->u.cb.call), event->u.cb.arg);
        g_module_close(entity->library.module);
        break;
